...
if(is_array($EFASS))
{
	...
	{
	?>
	var v<?php echo $leg->id?>_coords = new google.maps.LatLng(<?php echo $leg->lat?>, <?php echo $leg->lon?>);
	var v<?php echo $leg->id?>_marker = new google.maps.Marker({
			position: v<?php echo $leg->id?>_coords,
			map: map,
			icon: "<?php echo $icon; ?>",
			title: "<?php echo $leg->time.' '.$leg->stage; ?>",
			})
...
var flightPath2 = new google.maps.Polyline({
	path: [<?php if(count($list2) > 0) { echo implode(',', $list2); }?>],
